...
{		
    ...
    cvarFactor = CreateConVar("sm_remote_factor", "0.8 1 1.5", "Object Speed Factor: Specify either 1 factor (multiplied by (4 - upgrade level) or 3 values (one per upgrade level) separated with spaces");
    cvarSpeed = CreateConVar("sm_remote_speed", "680.0", "Speed at which remote objects move.");
    cvarJump = CreateConVar("sm_remote_jump", "2000.0", "Speed at which remote objects jump.");
    cvarFall = CreateConVar("sm_remote_fall", "600.0", "Speed at which remote objects fall.");
 ...
 //   RegConsoleCmd("sm_build_disp", Build, "Build a dispenser.");
   ...
   // RegAdminCmd("sm_remote_god", RemoteGod, ADMFLAG_ROOT, "Gives Sentry godmode (experimental).");
    ...
    // Build Limits
    cvarBuildEnabled  = CreateConVar("sm_buildlimit_enabled",        "0", "Enable/disable restricting buildings in TF2.");
...
DestroyBuilding(int i_object)
{
    if (IsValidEdict(i_object) && IsValidEntity(i_object))
    {
        SetVariantInt(1000);
        AcceptEntityInput(i_object, "RemoveHealth");
...
public Action:RemoteGod(client, args)
{
    ...
    else
    {
        ...
        if (i_object > 0 && IsValidEdict(i_object) && IsValidEntity(i_object))
        {
            if (GetEntProp(i_object, Prop_Send, "m_takedamage", 1)) // mortal
            {
                SetEntProp(i_object, Prop_Send, "m_takedamage", 0, 1);
                PrintToChat(client,"\x01\x04Building god mode on");
            }
            else // godmode
            {
                SetEntProp(i_object, Prop_Send, "m_takedamage", 1, 1);
...
public Action:Command_Build(client, args)
{
           ...
           (GetUserFlagBits(client) & (ADMFLAG_GENERIC|ADMFLAG_ROOT)) != 0))))
    {
        ...
        new TFExtObjectType:i_object = TFExtObjectType:StringToInt(sObject);
        new mode = StringToInt(sMode);

        new TFTeam:team = TFTeam:GetClientTeam(client);
        if (i_object < TFExtObject_Dispenser || i_object > TFExtObject_TeleporterExit || team < TFTeam_Red)
            return Plugin_Continue;

        new iCount = 0;
        if (!CheckBuild(client, i_object, mode, iCount))
            return Plugin_Handled;

        Call_StartForward(g_fwdOnBuildCommand);
        Call_PushCell(client);
        Call_PushCell(i_object);
        Call_PushCell(mode);
...
// Action Command_Foobar(int client, int args)
// {
//     // read args
//     // call function that actually do stuff
//     Foobar(client, a, b ,c)
// }


// void Foobar(int client, bool something, float another, int aaaaaaa)
// {
//     ...
// }
...
// void SomewhereElse()
// {
//     int client = GetClientFromSomewhere()
