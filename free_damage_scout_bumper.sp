...
#define DeathRacer 30686
#define TrackSuit 30685
...
stock GiveGiantPyro(client)
{
	if (IsValidClient(client))
	{
		...
		CreateRoboHat(client, DeathRacer, 10, 6, 0.0, 1.0, -1.0); 

		CreateRoboHat(client, TrackSuit, 10, 6, 0.0, 1.0, 0.0);
		...
		if(IsValidEntity(Threerune))
		{
			...
			TF2Attrib_SetByName(Threerune, "dmg penalty vs buildings", 0.25);
			 SetEntPropFloat(Threerune, Prop_Send, "m_flModelScale", 0.01);
		}

		TF2_AddCondition(client, TFCond_HalloweenKart);
		RequestFrame(SetHeadScale, client);
	}
}
