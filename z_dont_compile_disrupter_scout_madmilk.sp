...
TF2Attrib_SetByName(client, "health from packs decreased", HealthPackPickUpRate);
	...
	PrintHintText(client , "Spam Madmilk\nHit enemies with a fish");
...
stock GiveGiantPyro(client)
{
	if (IsValidClient(client))
	{
		...
		CreateRoboWeapon(client, "tf_weapon_jar_milk", 222, 6, 1, 1, 0);
		CreateRoboWeapon(client, "tf_weapon_bat_fish", 221, 6, 1, 2, 0);
		
		CreateRoboHat(client, Milkman, 10, 6, 0.0, 1.0, -1.0);
		//CreateHat(client, 219, 10, 6, false); //Milkman
...
//		CreateHat(client, 343, 10, 6, true);//Professor speks
		...
		if(IsValidEntity(Weapon1))
		{
			...
			TF2Attrib_SetByName(Weapon1, "effect bar recharge rate increased", 0.25);
