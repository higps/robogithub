...
#define ROBOT_NAME	"Ebony"
#define ROBOT_ROLE "Damage"
#define ROBOT_DESCRIPTION "MIRV V2 Rocket Launcher"
...
bool PlayerHasmirv_v2_[MAXPLAYERS+1];
bool RocketOverride[2049];
bool mirv_v2_Rocket[2049];
bool mirv_v2_Converge[2049];

bool Shouldmirv_v2_Converge;
...
public OnPluginStart()
{
	...
	g_rocketDiverge = CreateConVar("mirv_v2__split_variance", "20.0", "Random angle variance added onto mirv_v2_ rockets");
	g_showDebug = CreateConVar("mirv_v2__converge_debug", "0", "Show debug angles and trajectory for converging rockets", _, true, 0.0, true, 1.0);
	HookConVarChange(g_rocketCurve, Onmirv_v2_SettingsChanged);
	
	Shouldmirv_v2_Converge = GetConVarBool(g_rocketCurve);
...
//	AddCommandListener(PlayerJoinClass, "joinclass");
	...
	RegConsoleCmd("sm_mirv_v2_", CmdControl);
...
public Plugin MyInfo =
{
	name = "mirv_v2_ Rockets",
	author = "IvoryPal",
	description = "Rockets split into smaller rockets after a short delay."
};


public void Onmirv_v2_SettingsChanged(ConVar convar, char[] oldVal, char[] newVal)
{
	int cvarValue = StringToInt(newVal);
	Shouldmirv_v2_Converge = view_as<bool>(cvarValue);
}
...
// {
// 	if (TF2_GetPlayerClass(client) == TFClass_Soldier && PlayerHasmirv_v2_[client])
// 	{
// 		PlayerHasmirv_v2_[client] = false;
// 		PrintToChat(client, "[SM] Disabling mirv_v2_ rockets due to class change.");
// 	}
...
Action CmdControl(int client, int args)
{
	...
	{
		PlayerHasmirv_v2_[client] = !PlayerHasmirv_v2_[client];
		PrintToChat(client, "[SM] mirv_v2_ Rockets %s!", PlayerHasmirv_v2_[client] ? "enabled" : "disabled");
	}
}
...
public void OnEntityDestroyed(int entity)
{
	...
	if (IsValidEntity(entity))
	{
		...
		mirv_v2_Rocket[entity] = false;
		mirv_v2_Converge[entity] = false;
	}
}
...
void SplitRocket(int rocket, bool converge)
{
	...
	//PrintToChat(owner, "mirv_v2_ count: %i", count);
	for (int i = 1; i <= count; i++)
	{
		...
		int mirv_v2_ = CreateEntityByName(classname);
		mirv_v2_Rocket[mirv_v2_] = true;
		int team = GetClientTeam(owner);
		SetVariantInt(team);
		AcceptEntityInput(mirv_v2_, "TeamNum");
		AcceptEntityInput(mirv_v2_, "SetTeam");
		SetEntPropEnt(mirv_v2_, Prop_Send, "m_hOwnerEntity", owner);
		float vel[3];
		GetAngleVectors(angles, vel, NULL_VECTOR, NULL_VECTOR);
		ScaleVector(vel, speed);
		TeleportEntity(mirv_v2_, newPos, angles, vel);
		DispatchSpawn(mirv_v2_);
		if (HasEntProp(rocket, Prop_Send, "m_bCritical"))
			SetEntProp(mirv_v2_, Prop_Send, "m_bCritical", crit);
		SetEntDataFloat(mirv_v2_, FindSendPropInfo(netname, "m_iDeflected") + 4, 50.0);

		if (converge)
		{
			mirv_v2_Converge[mirv_v2_] = true;
			MinFlightTime[mirv_v2_] = GetEngineTime() + 1.0;
			ConvergePoint[mirv_v2_][0] = convergePos[0] += GetRandomFloat(-20.0, 20.0);
			ConvergePoint[mirv_v2_][1] = convergePos[1] += GetRandomFloat(-20.0, 20.0);
			ConvergePoint[mirv_v2_][2] = convergePos[2] += GetRandomFloat(-20.0, 20.0);
		}
	}
}
...
{
	if (IsValidEntity(rocket) && mirv_v2_Converge[rocket])
	{
