...
stock GiveGiantSoldier(client)
{
	if (IsValidClient(client))
	{
		...
		CreateWeapon(client, "tf_weapon_rocketlauncher", 228, 6, 1, 0, 0);
		
		CreateWeapon(client, "tf_weapon_buff_item", 354, 6, 1, 1, 0);
		
		CreateWeapon(client, "tf_weapon_katana", 357, 6, 1, 2, 0);
//		CreateWeapon(client, "tf_weapon_shovel", 447, 6, 1, 2, 0);
		
		SetEntPropFloat(client, Prop_Send, "m_flRageMeter", 100.0);
		
		CreateHat(client, 30177, 10, 6, true); //Hong kong cone
