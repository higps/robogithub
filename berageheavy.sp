...
#define ROBOT_DESCRIPTION "Rager"
...
/* 		PrintToChat(client, "1. You are now Giant Rage GPS!");
	...
	//}
	// else{
	// PrintToChat(client, "You have to be heavy to become Rage GPS");
...
stock GiveGRageH(client)
{
	if (IsValidClient(client))
	{
		...
		CreateWeapon(client, "tf_weapon_minigun", 15, 6, 1, 2);

		CreateHat(client, 1087, 10, 0, true);//machine gun mask
		//CreateHat(client, 479, 10, 6, true);//security shapdes
		CreateHat(client, 1088, 10, 6, false);//panzer
		CreateHat(client, 30815, 10, 6, false);//purity fist
		...
		if(IsValidEntity(Weapon1))
		{
			...
			//TF2Attrib_SetByName(Weapon1, "generate rage on damage", 5.0);
...
bool CreateHat(int client, int itemindex, int level, int quality, bool scale)
{
	...
	SetEntPropFloat(hat, Prop_Send, "m_flModelScale", 0.75);
	//AcceptEntityInput(hat, "SetModelScale", 0.75);
	}
