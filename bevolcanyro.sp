...
MakeGiantPyro(client)
{
	...
	PrintToChat(client, "1. You are now the Mighty Volcanyro!");
...
stock GiveGiantPyro(client)
{
	...
	{		
		...
		CreateWeapon(client, "tf_weapon_jar_gas", 1180, 6, 1, 2, 0);
		
		CreateWeapon(client, "tf_weapon_fireaxe", 348, 6, 1, 2, 0);

		CreateHat(client, 30038, 10, 6, true); //Firewall Helmet
		CreateHat(client, 30053, 10, 6, true); //Googol Glass Eyes
		CreateHat(client, 951, 10, 6, true);//Rail Spikes
				...
				if(IsValidEntity(Weapon3))
		{
			...
			//TF2Attrib_SetByName(Weapon3, "crit vs wet players", 1.0);
