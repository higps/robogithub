...
bool ActiveHasStatWeapon(int attacker)
{
	...
	if (!TF2CustAttr_GetString(iActiveWeapon, "team-hit-addcond", stat_buffer, sizeof(stat_buffer))) {
		return false;
	}
	g_TFCond = ReadIntVar(stat_buffer, "TFCond", -1);
	g_TFCond_duration = ReadFloatVar(stat_buffer, "duration", 5.0);
    g_TFCond_teammate = ReadIntVar(stat_buffer, "apply-to-teammate", 1);
...
public Action OnTraceAttack(int victim, int& attacker, int& inflictor, float& damage, int& damagetype, int& ammotype, int hitbox, int hitgroup)
{
	...
	&& ActiveHasStatWeapon(attacker))
	{
			...
			// if(g_bmod_disciplinary)TF2Attrib_AddCustomPlayerAttribute(attacker, "max health additive penalty", -150, 3.0);
