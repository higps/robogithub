...
void ThrowCleaver(int client, float flSpeed = 3000.0, int weapon) {
	...
	SetViewmodelAnimation(client, 33);

	PrecacheSound("weapons/cleaver_throw.wav");
	EmitSoundToAll("weapons/cleaver_throw.wav", client, SNDCHAN_AUTO, SNDLEVEL_NORMAL);
}
...
float g_fStockvecMin[3] = {-30.0, -30.0, -30.0};
float g_fStockvecMax[3] = {30.0, 30.0, 30.0};

void ChangeModel(int cleaver)
{

	SetEntityModel(cleaver, "models/weapons/c_models/c_battleaxe/c_battleaxe.mdl");
	SetEntPropFloat(cleaver, Prop_Send, "m_flModelScale", 2.0);
	SetEntPropVector(cleaver, Prop_Send, "m_vecMins", g_fStockvecMin);
	SetEntPropVector(cleaver, Prop_Send, "m_vecMaxs", g_fStockvecMax);
}
...
public Action OnCustomStatusHUDUpdate(int client, StringMap entries) {
	...
	for (int i; i < sizeof(g_flGunThrowRegenerateTime[]); i++) {
		...
		Format(buffer, sizeof(buffer), "Cleaver: %.0f%%",
