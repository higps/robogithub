/*
	...
	variables:
		damage = "cleaver_blast_damage" - float
		speed = "cleaver_grenade_speed" - float
		blast radius = "cleaver_blast_radius" - float
		regen time = "cleaver_regen_time"	- float
		det on impact = "cleaver_det_impact" - integer [0, 1]
*/
...
bool HasStat(int weapon, FCaber caber)
{
	...
	if (!TF2CustAttr_GetString(weapon, "cleaver-throw", stat_buffer, sizeof(stat_buffer)))
	{
		return false;
	}

	caber.Damage = ReadFloatVar(stat_buffer, "cleaver_blast_damage", 100.0);
	caber.Radius = ReadFloatVar(stat_buffer, "cleaver_blast_radius", 176.0);
	caber.DetOnImpact = view_as<bool>(ReadIntVar(stat_buffer, "cleaver_det_impact", 0));
	caber.RegenDuration = ReadFloatVar(stat_buffer, "cleaver_regen_time", 20.0);
	caber.Speed = ReadFloatVar(stat_buffer, "cleaver_grenade_speed", 1600.0);
...
void ThrowCaber(FClient client, int weaponId, FCaber caber)
{
	...
	PrecacheModel("models/weapons/c_models/c_battleaxe/c_battleaxe.mdl");
	...
	FObject grenade;
	grenade = FGameplayStatics.CreateObjectDeferred("tf_projectile_cleaver");
